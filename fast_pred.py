...
test_loader = data.DataLoader(test_data, batch_size=64, num_workers=4, shuffle=False, pin_memory=False, collate_fn=coco_collate_val)
...
for i, (images, path) in tqdm(enumerate(test_loader)):
    ...
    for j in range(preds.shape[0]):
        ...
        scores = min(1, to_np(torch.sum(nnOutputs[j])))
